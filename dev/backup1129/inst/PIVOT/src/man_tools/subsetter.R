...
observeEvent(input$plt_subset_btn, {
    ...
    withProgress(message = 'Subsetting', value = 0, {
        ...
        if(!rnorm) {
            ...
            result <- normalize_data(input, new_raw)
            if(is.null(result)) {
        ...
        r_data <- update_history(r_data, "Input data", "Subset", actionText, list(feature = r_data$feature_list, sample = r_data$sample_name), norm_method, r_data$norm_param)
...
observeEvent(input$undo_subset_sample, {
    withProgress(message = 'Processing', value = 0, {
        ...
        r_data$norm_param <- r_data$history[[1]]$norm_params
