...
output$select_data <- renderUI({
    if(is.null(input$data_folder)) return()
    datadir <-parseDirPath(roots=c(home='~'), input$data_folder)
...
observeEvent(input$submit_dir, {
    ...
    withProgress(message = 'Processing', value = 0, {
        ...
        result<-normalize_data(input, r_data$glb.raw)
        if(is.null(result)) {
            r_data <- init_state(r_data)
        ...
        r_data$glb.df <- result$df
        r_data$raw <- r_data$glb.raw
        r_data$df <- r_data$glb.df
        incProgress(0.2, detail = "Adding metadata...")
        r_data <- init_meta(r_data)
        r_data <- update_history(r_data, NA, "Input", "Input counts directory", list(feature = r_data$feature_list, sample = r_data$sample_name), r_data$norm_param$method, r_data$norm_param)
