...
observeEvent(input$range_filter_btn, {
    ...
    withProgress(message = 'Filtering', value = 0, {
        ...
        } else { # The data is from input data
            parent <- search_subset_node(r_data$his_tbl)
            if(!rnorm) {
                norm_method <- paste(r_data$history[[which(r_data$his_tbl$name == parent)]]$norm_param$method, "(inherit)")
            } else {
                norm_method <- paste(r_data$norm_param$method, "(re-norm)")
            }
        }
        
        r_data <- update_history(r_data, parent, "Filter", actionText, list(feature = r_data$feature_list, sample = r_data$sample_name, df = r_data$df), norm_method, r_data$norm_param)
...
observeEvent(input$undo_filter_feature, {
    withProgress(message = 'Processing...', value = 0, {
        ...
        r_data$norm_param <- r_data$history[[1]]$norm_param # return the normalization information to input
...
observeEvent(input$submit_marker, {
    ...
    withProgress(message = 'Filtering...', value = 0, {
        ...
        if(input$keep_filter) {
            ...
            parent <- search_subset_node(r_data$his_tbl)
            if(!rnorm) {
                print(which(r_data$his_tbl$name == parent))
                norm_method <- paste(r_data$history[[which(r_data$his_tbl$name == parent)]]$norm_param$method, "(inherit)")
            } else {
                norm_method <- paste(r_data$norm_param$method, "(re-norm)")
            }
