...
output$transdiff_ui <- renderUI({
    ...
    list(
        fluidRow(
            column(12, 
                   enhanced_box(
                       title = "Differential Expression Results for Gene Network Visualization",
                       id = "trans_de_tbl",
                       status = "primary",
                       solidHeader = T,
                       collapsible = T,
                       ...
                       register_analysis = T,
                       width = NULL,
                       tags$h4("Load DE table and compute gene score:"),
                       wellPanel(
                           wellPanel(
                               fluidRow(
                                   column(4, 
                                          selectInput("transdiff_species", "Choose species:", 
                   ...
                   enhanced_box(
                       title = "Network of Genes in the DE table",
                       id = "trans_net",
                       status = "warning",
                       solidHeader = T,
                       width = NULL,
                   ...
                   enhanced_box(
                       title = "Network Based Transcription Factor Scoring and Ranking",
                       id = "trans_result",
                       status = "danger",
                       solidHeader = T,
                       width = NULL,
                       ...
                       fluidRow(
                           column(12, 
                                  tags$b("Top Ranked TFs in Group 1"),
                       ...
                       fluidRow(
                           column(12, 
                                  tags$b("Top Ranked TFs in Group 2"),
...
output$tf_score_formula_ui <- renderUI({
    if(input$tf_score_type == "mogrify") {
        return(withMathJax2(HTML("$$Score = | Lfc | \\cdot (-\\log_{10} P_{adj}) $$")))
    }
    if(input$tf_score_type == "mogrify_modified") {
        return(withMathJax2(HTML("$$Score =
                                \\begin{cases}
    ...
    if(input$tf_score_type == "lfc") {
        return(withMathJax2(HTML("$$Score = | Lfc |$$")))
    }
...
# Load TF list
observe({
    if(is.null(input$transdiff_species) || is.null(r_data$df)) return()
    if(input$tf_custom_check) return()
    withProgress(message = 'Loading list...', value = 0.5, {
        if(input$transdiff_species == "9606"){
        ...
        } else if(input$transdiff_species == "10090") {
        ...
        r_data$tf_list <-r_data$input_tfs[which(toupper(r_data$input_tfs) %in% toupper(r_data$feature_list))]
...
output$tf_interactome_selection_ui <- renderUI({
    ...
    loaded_species<-names(species_list[which(species_list == input$transdiff_species)])
    interactome_list <- c(interactome_list, 
                          setNames(list("stringdb"), paste0("STRING (", loaded_species, ")")))
    
    if(input$transdiff_species %in% c("9606", "10090")) {
...
observeEvent(input$loadStringSpecies, {
    withProgress(message = 'Loading database...', value = 0.5, {
        tryCatch({
            r_data$stringdb <- STRINGdb::STRINGdb$new(version = "10", species = as.numeric(input$transdiff_species), score_threshold=as.numeric(input$stringdb_threshold))
            r_data$stringdb_species <- input$transdiff_species
            r_data$string_score <- as.numeric(input$stringdb_threshold)
            r_data$stringdb_g <- stringdb_get_graph(species=as.numeric(input$transdiff_species), version="10", score_threshold=as.numeric(input$stringdb_threshold))
        }, error = function(e){
        ...
        message_alert <- capture.output({
            ...
            r_data$string_meta <- string_meta[-which(duplicated(string_meta$gene)),] %>% dplyr::select(cap_name, gene, STRING_id) # remove duplicated(only keep first match), reduce size of the table
        })
...
output$net_vis <- renderUI({
    ...
    if(input$tf_interactome_selection == "reg" || input$net_vis_type == "igraph") {
        list(
            ...
            tags$p("The size of the vertices correlates with the absolute value of log fold change. Red: up-regulation. Blue: down-regulation.")
        )
...
output$tf_net_all <- renderPlot({
    ...
    if(input$tf_net_add_label) {
        ...
        legend("bottom", c(paste("Upregulated in Group 1"), paste("Upregulated in Group 2")), # Double check the direction
...
observe({
    if(!is.null(r_data$tfs)) {
        ...
        r_data$tf_tbl1 <- tfs %>% 
            ...
            dplyr::select(-one_of("pval", "padj", "lfc_or_es"))
