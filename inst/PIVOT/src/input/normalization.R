...
normalize_data <- function(method, params = NULL, raw) {
    ...
    if(grepl("RPKM", method)) {
        if(is.null(params$gene_len)) {
            session$sendCustomMessage(type = "showalert", "Please upload your gene lengths first.")
            return()
        }
        glen <- params$gene_len[match(toupper(rownames(raw)), toupper(names(params$gene_len)))]
        if(any(is.na(glen))) {
    ...
    if(method == "DESeq") {
        ...
        tryCatch({
            suppressWarnings({
                samplesAll <- data.frame(row.names=colnames(raw), celltype=rep("nt",length(colnames(raw))))
                dds <- DESeq2::DESeqDataSetFromMatrix(countData = raw, colData=samplesAll, design = ~ 1)
                sf_list <- estimateSizeFactorsForMatrix_MK(raw, threshold = (params$deseq_threshold)/100)
                DESeq2::sizeFactors(dds) <- sf_list$sf
                
                norm_param <- list(method = "Modified_DESeq", sizeFactor = data.frame(size_factor = sf_list$sf), numGenes = sf_list$numGenes, threshold = (params$deseq_threshold)/100)
                df <- as.data.frame(DESeq2::counts(dds, normalized=T))
            })
        }, 
        error = function(e){
    ...
    } else if (method %in% c("TMM", "upperquartile", "TMM-RPKM", "upperquartile-RPKM")) { # this is done using the edgeR function
        ...
        if(grepl("RPKM", method)) { # for RPKM, also need to add gene length vector
            norm_param <- list(method = method, sizeFactor = sf, gene_len = glen)
        } else {
